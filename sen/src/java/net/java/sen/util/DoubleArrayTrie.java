...
public class DoubleArrayTrie {
    ...
    {
        int begin = 0;
        int pos = (((((Node)children.get(0)).code + 1)>((int)next_check_pos))?(((Node)children.get(0)).code + 1):((int)next_check_pos)) - 1;
        ...
        while (true) {
            ...
            { int t = (int)(begin + ((Node)children.get(children.size()-1)).code); if(t>alloc_size) { resize((int)(t*1.05)); }};
            ...
            for (int i = 1; i < children.size(); i++) {
                if (array[(((int)begin + ((Node)children.get(i)).code) << 1) + 1] != 0) {
                    flag = true;
                    break;
                }
            }
        ...
        used[begin] = 1;
        size = (((size)>((int)begin + ((Node)children.get(children.size()-1)).code + 1))?(size):((int)begin + ((Node)children.get(children.size()-1)).code + 1));


        for (int i = 0; i < children.size(); i++) {
            array[(((int)begin + ((Node)children.get(i)).code) << 1) + 1] = begin;
        }
        ...
        for (int i = 0; i < children.size(); i++) {
            ...
            if (fetch(((Node)children.get(i)), new_children)==0) {
                array[((int)begin + (int)((Node)children.get(i)).code) << 1] =
                    (val!=null) ?
                    (int)(-val[((Node)children.get(i)).left]-1)
                    : (int)(-((Node)children.get(i)).left-1);

                if ((val != null) &&
                    ((int)(-val[((Node)children.get(i)).left]-1) >= 0)) {
                ...
                array[((int)begin + ((Node)children.get(i)).code) << 1] = ins;
            }
        }
