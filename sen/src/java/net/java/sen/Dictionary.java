 ...
 * USA
 *
 * $Id: Dictionary.java,v 1.1 2004-02-22 00:33:59 tora Exp $
 */
...
public class Dictionary {
		...
		for (int i = 0; i < n; i++) {
			...
			for (int j = 0; j < k; j++) {
				...
				if (log.isTraceEnabled()) {
					...
					log.trace(
						"posInfo="
							+ this.getPosInfo(result[size].posID));
		...
		if (n != -1) {
			...
			for (int i = 0; i < k; i++) {
				...
				if (log.isTraceEnabled()) {
					...
					log.trace("length=" + result[size].length);
					log.trace(
						"str=" + new String(str, pos, result[size].length));
				}
				size++;
			}
		}
	...
	public Dictionary(String tokenFile, 
